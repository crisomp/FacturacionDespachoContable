...
{
    public class ImprimirCreditoFiscal : ImpresoraHandler
    {
        ...
        int maxCar = 80, cortar;//Para una impresora ticketera que imprime a 40 columnas. La variable cortar cortara el texto cuando rebase el limte.
        ...
        public void TextoExtremos(string textoIzquierdo, string textoDerecho)
        {
            ...
            if (textoDerecho.Length > 40)//Si es mayor a 20 lo cortamos
            {
                cortar = textoDerecho.Length - 40;
                textoDer = textoDerecho.Remove(40, cortar);
            }
            else
        ...
        public void AgregaArticulo(string articulo, decimal cant, decimal precio, decimal importe)
        {
            ...
            if (cant.ToString().Length <= 5 && precio.ToString().Length <= 7 && importe.ToString().Length <= 8)
            {
                ...
                if (articulo.Length > 57)
                {
                    ...
                    for (int longitudTexto = articulo.Length; longitudTexto > 60; longitudTexto -= 60)
                    {
                        if (bandera == false)//si es false o la primera linea en recorrerer, continuar...
                        {
                            ...
                            linea.AppendLine(articulo.Substring(caracterActual, 60) + elemento);
                            bandera = true;//cambiamos su valor a verdadero
                        }
                        else
                            linea.AppendLine(articulo.Substring(caracterActual, 60));//Solo agrega el nombre del articulo

                        caracterActual += 60;//incrementa en 20 el valor de la variable caracterActual
                    }
                ...
                else //Si no es mayor solo agregarlo, sin dar saltos de lineas
                {
                    ...
                    for (int i = 0; i < (57 - articulo.Length); i++)
                    {
